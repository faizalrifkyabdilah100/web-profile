...
<div>
    ...
    <section class="relative pt-24 pb-16 lg:pt-32 lg:pb-24 flex items-center justify-center bg-gradient-to-br from-blue-900 via-blue-800 to-indigo-900 overflow-hidden text-center">
        ...
        <div class="relative z-10 text-white px-4 max-w-4xl mx-auto" data-aos="zoom-in" data-aos-duration="1000">
            <h1 class="text-4xl md:text-6xl mb-6 font-bold tracking-tight">Edit Mata Pelajaran</h1>
            <p class="text-xl text-blue-100 max-w-2xl mx-auto font-light leading-relaxed">
                Perbarui informasi mata pelajaran <?= esc($mapel['nama']) ?>
            </p>
        </div>
    </section>
            ...
            <div class="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden" data-aos="fade-up" data-aos-delay="100">
                ...
                <div class="bg-gradient-to-r from-amber-500 to-orange-500 px-8 py-6">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h2 class="text-2xl font-bold text-white">Edit Mata Pelajaran</h2>
                            <p class="text-amber-100 text-sm">Perbarui data mata pelajaran dengan informasi terbaru</p>
                        </div>
                    </div>
                </div>

                <form action="<?= base_url('mata-pelajaran/update/' . $mapel['id']) ?>" method="post" class="p-8">
                    <?= csrf_field() ?>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="nama" id="nama" value="<?= old('nama', $mapel['nama']) ?>" 
                                       class="w-full pl-12 pr-4 py-3 bg-slate-50 border-2 <?= $validation->hasError('nama') ? 'border-red-300 bg-red-50' : 'border-gray-200' ?> rounded-xl focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 transition-all duration-300 outline-none"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="kode" id="kode" value="<?= old('kode', $mapel['kode']) ?>" 
                                       class="w-full pl-12 pr-4 py-3 bg-slate-50 border-2 border-gray-200 rounded-xl focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 transition-all duration-300 outline-none"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <select name="kelompok" id="kelompok" 
                                        class="w-full pl-12 pr-4 py-3 bg-slate-50 border-2 border-gray-200 rounded-xl focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 transition-all duration-300 outline-none appearance-none">
                                    <option value="">Pilih Kelompok</option>
                                    <option value="Kelompok A (Umum)" <?= old('kelompok', $mapel['kelompok']) === 'Kelompok A (Umum)' ? 'selected' : '' ?>>Kelompok A (Umum)</option>
                                    <option value="Kelompok B (Umum)" <?= old('kelompok', $mapel['kelompok']) === 'Kelompok B (Umum)' ? 'selected' : '' ?>>Kelompok B (Umum)</option>
                                    <option value="Muatan Lokal" <?= old('kelompok', $mapel['kelompok']) === 'Muatan Lokal' ? 'selected' : '' ?>>Muatan Lokal</option>
                                    <option value="Pengembangan Diri" <?= old('kelompok', $mapel['kelompok']) === 'Pengembangan Diri' ? 'selected' : '' ?>>Pengembangan Diri</option>
                                </select>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <div class="relative">
                                ...
                                <textarea name="deskripsi" id="deskripsi" rows="3" 
                                          class="w-full pl-12 pr-4 py-3 bg-slate-50 border-2 border-gray-200 rounded-xl focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 transition-all duration-300 outline-none resize-none"
                                          placeholder="Deskripsi singkat mata pelajaran"><?= old('deskripsi', $mapel['deskripsi']) ?></textarea>
                            </div>
                        </div>
                    </div>
